use std::cell::RefCell;
use std::collections::HashMap;
use std::sync::mpsc::{Receiver, Sender, channel};
use shaders::Shape;
use wgpu::BufferSize;
use crate::{IntoShape, Tensor, TensorEncoder};

pub(crate) struct TensorPool {
    sender: Sender<Tensor<'static>>,
    receiver: Receiver<Tensor<'static>>,
    tensors: RefCell<HashMap<BufferSize, HashMap<Shape, Vec<Tensor<'static>>>>>
}

impl TensorPool {
    pub(crate) fn new() -> TensorPool {
        let (sender, receiver) = channel();
        TensorPool {
            tensors: RefCell::new(HashMap::new()),
            receiver,
            sender
        }
    } 
}

impl<'scope> TensorEncoder<'scope> {
    pub(crate) fn temp<'a>(
        &'a mut self,
        shape: impl IntoShape,
    ) -> Tensor<'scope> {
        if let Ok(mut tensors) = self.tensors.tensors.try_borrow_mut() {
            while let Ok(tensor) = self.tensors.receiver.try_recv() {
                let size = BufferSize::new(tensor.buffer.size());
                let Some(size) = size else { continue };
                tensors.entry(size).or_default()
                    .entry(tensor.shape())
                    .or_default()
                    .push(tensor)
            }
        }

        let shape = shape.shape();
        let bucket = Tensor::bucket_size(shape);
 
        if let Ok(mut tensors) = self.tensors.tensors.try_borrow_mut() {
            if let Some(tensors) = tensors.get_mut(&bucket) {
                if let Some(mut tensor) = Some(shape)
                    .filter(|_| tensors.contains_key(&shape))
                    .or(tensors.keys().next().cloned())
                    .map(|shape| tensors.get_mut(&shape))
                    .flatten().map(Vec::pop).flatten() {

                    Some(shape).filter(|_| tensors.contains_key(&shape))
                        .or(tensors.keys().next().cloned())
                        .filter(|s| tensors.get(s)
                            .map(Vec::is_empty)
                            .unwrap_or(false)
                        ).map(|shape| tensors.remove(&shape));

                    if tensor.shape != shape {
                        self.reshape(&tensor, shape);
                        tensor.shape = shape;
                    }
                    tensor.sender = Some(&self.tensors.sender);
                    return tensor;
                }
            }
        } 

        let mut tensor = Tensor::create(
            self.bind_group_layouts,
            self.device, shape, bucket
        );
        tensor.sender = Some(&self.tensors.sender);
        tensor
    }
}